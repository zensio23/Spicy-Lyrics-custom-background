...
if defined SPICETIFY_EXE (
  if exist "%SPICETIFY_EXE%" (
    set "SPICETIFY_CMD=%SPICETIFY_EXE%"
  ) else (
    echo [ERROR] SPICETIFY_EXE is set, but the file does not exist:
    echo         "%SPICETIFY_EXE%"
    goto finish_error
  )
)

if not defined SPICETIFY_CMD (
  where spicetify >nul 2>&1
  if not errorlevel 1 (
    set "SPICETIFY_CMD=spicetify"
  )
)
...
if not defined SPICETIFY_CMD (
  ...
  echo If Spicetify is installed in a custom location, set SPICETIFY_EXE to its full path and rerun the installer.
  goto finish_error
)
...
call :resolve_spicetify_dir
set "SPICETIFY_CONFIG_FILE=%SPICETIFY_DIR%\config-xpui.ini"
...
if not defined SKIP_BUILD if exist "%SCRIPT_DIR%bun.lock" (
  if defined BUN_EXE (
    if exist "%SPICETIFY_CONFIG_FILE%" (
      ...
      if defined REQUIRE_BUILD (
        echo [ERROR] Spicetify config is missing:
        echo         "%SPICETIFY_CONFIG_FILE%"
        echo The source build needs a working Spicetify installation before it can compile this project.
        echo Run Spicetify once or execute "spicetify apply", then rerun this script.
        goto finish_error
      )

      echo [INFO] Spicetify config was not found yet.
      echo [INFO] The installer will use the bundled release files instead of rebuilding from source.
      echo [INFO] This keeps first-time GitHub ZIP installs working even if a local creator build is not ready yet.
    )
  ) else (
    if defined REQUIRE_BUILD (
...
call :run_spicetify config extensions %EXTENSION_FILE_MJS%- >nul 2>&1
call :run_spicetify config extensions %EXTENSION_FILE_JS%- >nul 2>&1
call :run_spicetify config extensions %EXTENSION_FILE_JS%
if errorlevel 1 (
...
:resolve_spicetify_dir
set "SPICETIFY_DIR="
set "SPICETIFY_OUTPUT_FILE=%TEMP%\spicy-lyrics-spicetify-userdata-%RANDOM%-%RANDOM%.txt"
if exist "%SPICETIFY_OUTPUT_FILE%" del /q "%SPICETIFY_OUTPUT_FILE%" >nul 2>&1

call :run_spicetify path userdata > "%SPICETIFY_OUTPUT_FILE%" 2>nul
if exist "%SPICETIFY_OUTPUT_FILE%" (
  for /f "usebackq delims=" %%I in ("%SPICETIFY_OUTPUT_FILE%") do (
    if not defined SPICETIFY_DIR set "SPICETIFY_DIR=%%I"
  )
  del /q "%SPICETIFY_OUTPUT_FILE%" >nul 2>&1
)
