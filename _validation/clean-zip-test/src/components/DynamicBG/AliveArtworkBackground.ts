...
function resolveGlowColor(
  ...
  const lift =
    (0.05 + score * 0.08 + saturation * 0.05) *
    (1 - smoothstep(0.72, 0.97, lightness) * 0.72);
  return [
...
export class AliveArtworkBackground {
  ...
  private smoothSignals(
    ...
    const response = scaleAroundNeutral(settings.response, 2.2, 0.45);
    const attackMs = 92 * response;
    const releaseMs = 360 * response;
    ...
    const loudnessGate = smoothstep(loudnessThreshold, 0.92, snapshot.absoluteLoudness);
    const gatedLoudness = clamp(
      Math.max(0, snapshot.absoluteLoudness - loudnessFloor) * lerp(0.52, 1.08, loudnessGate)
    );
    ...
    this.visualSignals.impact = approach(
      ...
      clamp(snapshot.impact * (0.6 + loudnessGate * 0.8)),
      deltaMs,
      attackMs * 0.68,
      releaseMs * 0.52
    );
  ...
  private updateKawarpSpeed(
    ...
    const movementScale = scaleAroundNeutral(settings.movementSpeed, 0.35, 2.8);
    const beatdropScale = scaleAroundNeutral(settings.beatdropMultiplier, 0.55, 3.2);
    const dropDriveRaw =
      this.visualSignals.drop * 0.42 +
      this.visualSignals.impact * 0.22 +
      this.visualSignals.beat * 0.16 +
      this.visualSignals.transient * 0.08;
    const dropDrive = smoothstep(0.1, 0.72, dropDriveRaw);
    const dynamicBeatdrop = lerp(1, beatdropScale, clamp(dropDrive));
    const pausedBase = snapshot.isPlaying ? 1 : 0.1;
    const baseSpeed = snapshot.baseAnimationSpeed || pausedBase;
    const speed = clamp(baseSpeed * movementScale * dynamicBeatdrop, 0.06, 3);
  ...
  private renderLights(settings: BackgroundVisualSettings, deltaMs: number, now: number) {
    ...
    const waveGlowScale = scaleAroundNeutral(settings.waveGlowStrength, 0.32, 3.6);
    const beatBrightnessScale = scaleAroundNeutral(settings.beatBrightness, 0.45, 3.2);
    const impactScale = scaleAroundNeutral(settings.dropImpact, 0.5, 3.1);
    const glowLimitScale = mapAroundNeutral(settings.glowLimit, 0.14, 0.07, 0.36);
    ...
    const fieldEnergy = clamp(
      this.visualSignals.loudness * 0.18 +
        this.visualSignals.bass * 0.2 +
        this.visualSignals.lowMid * 0.12 +
        this.visualSignals.beat * 0.16 +
        this.visualSignals.drop * 0.22 +
        this.visualSignals.transient * 0.08 +
        this.visualSignals.presence * 0.04 +
        this.visualSignals.vocal * 0.06
    );
    const pulseEnergy = clamp(
      this.visualSignals.beat * 0.34 +
        this.visualSignals.drop * 0.32 +
        this.visualSignals.transient * 0.16 +
        this.visualSignals.bass * 0.1 +
        this.visualSignals.loudness * 0.08
    );
    const chorusLift = clamp(
      this.visualSignals.drop * 0.48 +
        this.visualSignals.impact * 0.32 +
        this.visualSignals.loudness * 0.14 +
        this.visualSignals.beat * 0.08
    );
    const sceneEnergy = clamp(fieldEnergy * 0.72 + pulseEnergy * 0.18 + chorusLift * 0.56);
    const localParticipation = smoothstep(0.1, 0.34, sceneEnergy);
    const broadParticipation = smoothstep(0.28, 0.62, sceneEnergy + chorusLift * 0.18);
    const peakParticipation = smoothstep(0.52, 0.92, chorusLift + sceneEnergy * 0.28);
    const activeRegionFraction = clamp(
      0.16 +
        waveGlowPositive * 0.05 +
        localParticipation * 0.18 +
        broadParticipation * (0.16 + impactPositive * 0.14) +
        peakParticipation * (0.22 + impactPositive * 0.2) -
        impactNegative * 0.16,
      0.12,
      1
    );
    const fieldFloor = clamp(
      fieldEnergy *
        localParticipation *
        (0.045 + waveGlowPositive * 0.12 + brightnessPositive * 0.04 + impactPositive * 0.025) *
        lerp(1, 0.5, waveGlowNegative),
      0,
      0.26
    );
    const chorusField = clamp(
      (chorusLift * 0.72 + broadParticipation * 0.08 + peakParticipation * 0.14) *
        (0.03 + impactPositive * 0.1 + waveGlowPositive * 0.05),
      0,
      0.28
    );
    ...
    for (const emitter of this.emitters) {
      ...
      const colorSignal =
        ...
        (0.74 + emitter.featureScore * 0.4);
      const activationEnergy =
        sceneEnergy * 0.78 + pulseEnergy * 0.22 + emitter.featureScore * 0.08;
      const activationGate = smoothstep(
        emitter.sensitivity - (0.18 + impactPositive * 0.14),
        emitter.sensitivity + (0.22 - impactNegative * 0.06),
        activationEnergy
      );
      ...
      const rawTarget = clamp(
        (fieldFloor * waveGlowScale +
          ...
          colorSignal *
            (0.14 +
              waveGlowPositive * 0.22 +
              brightnessPositive * 0.06 +
              localParticipation * 0.05) +
          fieldEnergy * waveGlowScale * (0.08 + broadParticipation * 0.18) +
          pulseEnergy * impactScale * (0.12 + peakParticipation * 0.06)) *
          emitter.intensity *
          beatBrightnessScale *
          (0.84 + emitter.featureScore * 0.22) *
          activationGate
      );
    ...
    for (const [index, emitter] of this.emitters.entries()) {
      ...
      const regionalGate =
        cutoff <= 0
          ...
          : smoothstep(
              ...
              Math.max(0.001, cutoff * lerp(1.06, 0.9, peakParticipation)),
              rawTarget + fieldFloor * (0.22 + broadParticipation * 0.28) + chorusField * 0.34
            );
      const floorTarget =
        (fieldFloor * (0.42 + emitter.featureScore * 0.2) +
          chorusField * (0.22 + peakParticipation * 0.18)) *
        lerp(1, 0.58, waveGlowNegative);
      const target = clamp(
        Math.max(floorTarget, rawTarget * regionalGate) *
          (0.88 + emitter.featureScore * 0.18) *
          lerp(1, 1.42, peakParticipation)
      );
      ...
      emitter.current = approach(
        ...
        target,
        deltaMs,
        68 / emitter.fadeSpeed,
        260 * emitter.fadeSpeed
      );
    ...
    const fieldBloom =
      0.56 + waveGlowPositive * 0.34 + impactPositive * 0.2 + peakParticipation * 0.18;
    ...
    for (const emitter of this.emitters) {
      ...
      const motion = 0.08 + this.visualSignals.movement * 0.16;
      const offsetX =
      ...
      const radius = Math.max(
        ...
        emitter.radius *
          ...
          (0.86 + fieldBloom * 0.24 + this.visualSignals.beat * 0.12 + this.visualSignals.drop * 0.24)
      );
      const haloRadius = radius * (1.18 + waveGlowPositive * 0.26 + peakParticipation * 0.2);
      const pulseRadius =
        radius * (0.74 + this.visualSignals.beat * 0.06 + this.visualSignals.drop * 0.12);

      const [red, green, blue] = emitter.glowColor;
      const outerAlpha = clamp(
        intensity *
          (0.05 +
            waveGlowPositive * 0.08 +
            brightnessPositive * 0.05 +
            peakParticipation * 0.09),
        0,
        0.3
      );
      const pulseAlpha = clamp(
        intensity *
          (0.04 +
            brightnessPositive * 0.07 +
            pulseEnergy * 0.045 +
            peakParticipation * 0.04 +
            this.visualSignals.presence * 0.01),
        0,
        0.22
      );
